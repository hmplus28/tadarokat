...
def _vectorized_flow_status(df: pd.DataFrame, expert: Optional[str] = None) -> pd.Series:
    ...
    pns = df["شماره"].map(_normalize_id)
    titles = (
...
def _filter_purchases(df: pd.DataFrame, filter_type: str) -> pd.DataFrame:
    if filter_type == "purchase":
        return df[df["وضعیت"].isin(["در جریان", "تایید شده", "ثبت شده"])]
    if filter_type == "inquiry":
        if "has_local_inquiry" in df.columns:
            return df[df["has_local_inquiry"].fillna(False).astype(bool)]
        return df.iloc[0:0]
    if filter_type == "no_inquiry":
        if "has_local_inquiry" in df.columns:
            mask = df["has_local_inquiry"].fillna(False).astype(bool)
            if "inquiry_approved" in df.columns:
                approved = df["inquiry_approved"].fillna(False).astype(bool)
                return df[~mask & ~approved]
            return df[~mask]
        return df
    if filter_type == "returned":
        return df[df["وضعیت"] == "معلق"]
    if filter_type == "closed":
        return df[df["وضعیت"] == "بسته شده"]
    return df
...
def _stats_from_df(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "purchase": int(df["وضعیت"].isin(["در جریان", "تایید شده", "ثبت شده"]).sum()),
        "inquiry": int(has_inquiry.sum()),
        "returned": int((df["وضعیت"] == "معلق").sum()),
        "closed": int((df["وضعیت"] == "بسته شده").sum()),
        "in_progress": int((df["وضعیت"] == "در جریان").sum()),
        "by_status": _value_counts_dict(df["وضعیت"]),
        "by_expert": _value_counts_dict(df["کارشناس خرید"], dropna=False),
        "by_type": _value_counts_dict(df["نوع خرید"], dropna=False),
    }
...
def _summary_from_df(df: pd.DataFrame) -> dict:
    return {
        ...
        "status_breakdown": _value_counts_dict(df["وضعیت"]),
        "purchase_type_breakdown": _value_counts_dict(df["نوع خرید"], dropna=False),
        "urgency_breakdown": _value_counts_dict(df["رمز فوریت"], dropna=False),
        "monthly_trend": _value_counts_dict(df["تاریخ"].astype(str).str[:7], dropna=False),
    }
...
def get_reorder_report(page: int = 1, page_size: int = 50, expert: Optional[str] = None) -> Dict:
    df = _get_merged_purchases()
    df = _filter_expert_purchases(df, expert)
    active = df[df["وضعیت"].isin(["در جریان", "تایید شده", "معلق"])]
