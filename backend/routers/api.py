...
def refresh_data_from_db(user: dict = Depends(get_current_user)):
    ...
    changed = False
    try:
        changed = excel_service.refresh_if_excel_changed()
        # ✅ Sync deliveries and orders from Excel
        from services.excel_service import sync_deliveries_from_excel, sync_orders_from_excel
        print("[INFO] Starting sync from Excel...")
        sync_deliveries_from_excel(user.get("username") or "system")
        sync_orders_from_excel(user.get("username") or "system")
    ...
    return {
        ...
        "excel_changed_detected": changed,
        "database": db_info,
...
@router.get("/reports/qty-delivered")
def qty_delivered_report(
    ...
    if user.get("role") not in ("admin", "manager"):
        raise HTTPException(status_code=403, detail="فقط مدیر و سوپر ادمین")
    return excel_service.get_qty_vs_delivered_report(date_from=date_from, date_to=date_to, expert=expert)
...
def requests(
    ...
    resp = excel_service.get_purchase_requests(
        search=search,
        filter_type=filter,
    ...
    if user.get("role") == "warehouse":
        # Warehouse sees only minimal fields for status update
        limited = ["شماره", "نام قلم خریدنی", "کد قلم خریدنی", "مقدار درخواست", "واحد سنجش", "کارشناس خرید", "وضعیت", "وضعیت فعلی خرید", "تاریخ"]
...
def request_detail(request_number: str, user: dict = Depends(get_current_user)):
    ...
    if user.get("role") == "warehouse" and row:
        limited = ["شماره", "نام قلم خریدنی", "کد قلم خریدنی", "مقدار درخواست", "واحد سنجش", "کارشناس خرید", "وضعیت", "وضعیت فعلی خرید", "تاریخ"]
        row = {k: v for k, v in row.items() if k in limited}
    return row


@router.patch("/requests/{request_number}")
def update_request(request_number: str, payload: dict, user: dict = Depends(get_current_user)):
    if user.get("role") not in ("admin", "manager", "expert", "warehouse"):
        raise HTTPException(status_code=403)
    if user.get("role") == "warehouse":
        # warehouse can only update وضعیت
        allowed = {"وضعیت", "وضعیت فعلی خرید"}
        if not allowed.intersection(payload.keys()):
            raise HTTPException(status_code=400, detail="فقط بروزرسانی وضعیت مجاز است")
        # strip other keys
        payload = {k: v for k, v in payload.items() if k in allowed}
    try:
        return excel_service.update_purchase_request(
            request_number, payload, user["username"], admin=user.get("role") in ("admin", "manager"),
        )
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
