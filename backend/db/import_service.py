...
class ImportService:
        ...
        with self.new_mgr.connect(write=True) as new_conn:
            ...
            if DB_CURRENT_PATH.exists():
                with self.current_mgr.connect(write=False) as cur_conn:
                    ...
                    for key in ("db_version", "last_import_at", "last_import_sha256"):
    ...
    def atomic_swap(self) -> Dict[str, Any]:
        ...
        try:
            # بستن WAL قبل از rename
            for suffix in ("-wal", "-shm"):
                for base in (DB_CURRENT_PATH, DB_OLD_PATH):
            ...
            # تلاش rollback: اگر current حذف شده و old هست
            if not DB_CURRENT_PATH.exists() and DB_OLD_PATH.exists():
                try:
