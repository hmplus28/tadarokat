...
| [**راهنمای تست یک‌کلیک**](docs/TEST_SETUP.md) | تست‌کننده / توسعه‌دهنده | `test_run.bat`، `input.xlsx`، کاربران تست |
...
می‌خواهید **بدون تنظیم دستی** share و seed، سریع برنامه را روی PC خود تست کنید؟

1. **`input.xlsx`** را در **ریشه پروژه** بگذارید (کنار `test_run.bat`)
2. دوبار کلیک **`test_run.bat`** (ویندوز)
3. بعد از `TEST SETUP COMPLETE`، مرورگر باز می‌شود → http://127.0.0.1:8000
4. ورود: `admin` / `admin123`

> راهنمای **گام‌به‌گام با جزئیات** (پیش‌نیازها، لینوکس، خط فرمان، عیب‌یابی):  
> **[docs/TEST_SETUP.md](docs/TEST_SETUP.md)**

> هر اجرای `test_run.bat` دیتابیس را از نو می‌سازد. برای نصب واقعی به [گام ۳](#گام-۳--نصب-برنامه-یک-بار-روی-هر-pc) بروید.
...
```
TEST_SETUP (تست سریع لوکال با test_run.bat)
    ↓
