...
function canIssueInquiry(row) {
  ...
  // ✅ چک ساده: اگر شماره استعلام در اکسل پر باشد، نمی‌توان استعلام زد
  const inquiryNumber = row["شماره استعلام"];
  if (inquiryNumber && String(inquiryNumber).trim() && String(inquiryNumber).trim() !== "—" && String(inquiryNumber).trim() !== "None" && String(inquiryNumber).trim() !== "nan") {
    return false;
  }
