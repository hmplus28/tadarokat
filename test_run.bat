...
def run_install() -> None:
    """Install dependencies using default PyPI (no mirror, no inherited mirror settings)."""
    ...
    # Copy environment and wipe out any mirror variables that may have leaked from a batch file
    pip_env = os.environ.copy()
    for mir_var in ("PIP_INDEX_URL", "PIP_TRUSTED_HOST", "PIP_EXTRA_INDEX_URL"):
        pip_env.pop(mir_var, None)

    # Set only timeout/retry, no index-url override → default PyPI
    pip_env["PIP_DEFAULT_TIMEOUT"] = "120"
    pip_env["PIP_RETRIES"] = "5"
